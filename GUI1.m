...
% Last Modified by GUIDE v2.5 07-Jun-2018 15:17:53
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
str = {'cross section','beam','column','frame','animation of Temp in cross section','analysis in frames','test1','help','test2'};
...
function func1(handles)
    global a b c d e %input from GUI
%     x = get(handles.diyigeshu,'String');
%     y = get(handles.diergeshu,'String');
%     a = str2double(x);
%    
...
generateMesh(A,'Hmax',0.2); %create mesh with scale 
...
thermalBC(B,'Edge',5,'HeatFlux',-.22);

thermalBC(B,'Edge',7,'HeatFlux',-1.09);
thermalBC(B,'Edge',8,'HeatFlux',-1.09);
thermalBC(B,'Edge',11,'HeatFlux',-.22);
...
getnode = @(p,x,y) min((p(1,:) - x).^2 + (p(2,:) - y).^2);
 
[~,nid] = getnode( msh.Nodes, .5, 0 );
...
thermalProperties(B,'SpecificHeat',FCAPA ,'ThermalConductivity',FCON,'MassDensity',den);                           
...
txt = {'First input box is for the temperature on the heated surface.';
       ...
       'the following three are for the coefficient of the object we use';
...
% point3 = [3 4 .075 .0075 .0075 .075  -.0975 -.0975 .0975 .0975];
 ...
 point1 = [3 4 0 0.05 0.05 0 0 0 0.4 0.4];
 point2 = [3 4 0.024 0.026 0.026 0.024 0.1999 0.1999 0.2001 0.2001]; 
 


reverse = [point1;point2]';
g = decsg(reverse,'R1-R2',['R1';'R2']');
geometryFromEdges(A,g);
 


thermalBC(A,'Edge',1,'Temperature',b);
thermalBC(A,'Edge',2,'Temperature',b);
thermalBC(A,'Edge',7,'Temperature',b);
thermalBC(A,'Edge',8,'Temperature',b);
thermalBC(A,'Edge',5,'Temperature',a);
thermalBC(A,'Edge',3,'Temperature',a);
thermalBC(A,'Edge',4,'Temperature',a);
thermalBC(A,'Edge',6,'Temperature',a);
thermalBC(A,'Edge',1,'HeatFlux',-22.22);
thermalBC(A,'Edge',2,'HeatFlux',-22.22);
thermalBC(A,'Edge',6,'HeatFlux',-22.22);
thermalBC(A,'Edge',7,'HeatFlux',-21.09);


thermalProperties(A,'ThermalConductivity',515);
 
 
generateMesh(A,'Hmax',0.002);
...
g = decsg(reverse,'R1-R2',['R1';'R2']');
geometryFromEdges(B,g);
 thermalBC(B,'Edge',1,'Temperature',b);
thermalBC(B,'Edge',2,'Temperature',b);
thermalBC(B,'Edge',7,'Temperature',b);
thermalBC(B,'Edge',6,'Temperature',b);
thermalBC(B,'Edge',5,'Temperature',a);
thermalBC(B,'Edge',3,'Temperature',a);
thermalBC(B,'Edge',4,'Temperature',a);
thermalBC(B,'Edge',8,'Temperature',a);
 thermalBC(B,'Edge',1,'HeatFlux',-222);
 thermalBC(B,'Edge',2,'HeatFlux',-222);
% thermalBC(B,'Edge',3,'HeatFlux',-33.3);
% thermalBC(B,'Edge',4,'HeatFlux',-33.3);
% thermalBC(B,'Edge',5,'HeatFlux',-2.22);
thermalBC(B,'Edge',6,'HeatFlux',-222);
thermalBC(B,'Edge',7,'HeatFlux',-210);
% thermalBC(B,'Edge',8,'HeatFlux',-21.09);
...
numframes = 400;
h = figure;
h.Position = [1 0.5 1 1].*h.Position;
subplot(1,2,1);
axis equal
pdeplot(B,'XYData',T(:,end),'Contour','on','ColorMap','jet');
axis equal
...
for k = 1 :numframes
    subplot(1,2,2)
    pdeplot(B,'XYDATA',T(:,k),'ColorMap','jet');
    caxis([Tmin Tmax]);
    title(['Temp at ',num2str(tlist(k)),'seconds']);
    mv(k)=getframe;
end
movie(mv,1)


function func10(handles)
A = createpde('thermal','steadystate');
point1 = [3 4 0 0.3 0.3 0 0 0 0.4 0.4];

reverse = [point1]';

g = decsg(reverse,'R1',['R1']');
geometryFromEdges(A,g);

%figure
%pdegplot(A,'EdgeLabels','on');

thermalBC(A,'Edge',4,'HeatFlux',0);
thermalBC(A,'Edge',3,'Temperature',1000);
thermalBC(A,'Edge',2,'Temperature',1000);
thermalBC(A,'Edge',1,'Temperature',1000);
 
thermalProperties(A,'ThermalConductivity',52);
 
 
generateMesh(A,'Hmax',0.02);
% figure
% pdeplot(A);
% axis equal
...
thermalProperties(B,'MassDensity', 7850,'SpecificHeat',460, 'ThermalConductivity',52);
                             
                                
  
thermalBC(B,'Edge',4,'HeatFlux',0);
thermalBC(B,'Edge',3,'Temperature',1000);
thermalBC(B,'Edge',2,'Temperature',1000);
thermalBC(B,'Edge',1,'Temperature',1000);
...
[~,nid] = getClosestNode( msh.Nodes, .1, .3 );
